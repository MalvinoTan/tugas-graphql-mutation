...
/** Queries */
import { DELETE_TODO, GET_TODOS, GET_TODO_BY_ID, INSERT_TODO, UPDATE_CHECKED_TODO, UPDATE_TITLE_TODO } from "./GraphQL/Todos/queries";
...
const App = () => {
  ...
  const [getTodoById, { data: dataId, loading: loadingId, error: errorId }] = useLazyQuery(GET_TODO_BY_ID);
  ...
  const [insertTodo] = useMutation(INSERT_TODO, {
    ...
    onError: (error) => {
      alert(`Terdapat error saat insert!!!\n${error}`);
    }
  });
  ...
  const [updateCheckedTodo] = useMutation(UPDATE_CHECKED_TODO, {
    onCompleted: (data) => {
      refetch();
    },
    onError: (error) => {
      alert(`Terdapat error saat update checked!!!\n${error}`);
    }
  });

  const [updateTitleTodo] = useMutation(UPDATE_TITLE_TODO, {
    onCompleted: (data) => {
      refetch();
    },
    onError: (error) => {
      alert(`Terdapat error saat update title!!!\n${error}`);
    }
  });
  ...
  const onSubmitList = (e) => {
    ...
    insertTodo({ variables: { title: inputs.submit } });
  ...
  const onClickItem = (id) => {
    data.todos.forEach(item => {
      if (item.id === id) {
        updateCheckedTodo({ variables: { id: id, isDone: !item.isDone } });
      }
    });
  };
    ...
    <>
      <div className="container">
        ...
        <form className="js-form" onSubmit={onSubmitList}>
          <input
            onChange={(e) => setInputs({ ...inputs, submit: e.target.value })}
            value={inputs.submit}
            autoFocus
            type="text"
